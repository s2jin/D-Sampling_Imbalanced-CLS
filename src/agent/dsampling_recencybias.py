...
from .recencybias import recency_sampler as recency
...
class Agent():
    ...
    def fit(self): ## TRAINING
        ...
        self.recency_sampler = recency.Sampler(
                self.train_dataloader.dataset.data_size, 
                len(self.train_dataloader.dataset.label_list), 
                self.config.agent.warmup, ## paper magic num: 10 
                [100.0,1.0], 
                [self.config.agent.warmup, self.config.agent.epochs]) ## (num_train, num_lbl, queue_size, s_es, epochs)
        def set_sampler(epoch=None):
            if epoch is not None:
                logging.info('Update sampler')
                self.recency_sampler.update_sampling_probability(epoch+1, self.train_dataloader.dataset.data, normalize=True)
            self.model.eval()
            iloss, iacc, logits, ids = 0,0,list(),list()
            ...
            self.recency_sampler.async_update_prediction_matrix(ids, logits)
            sampler = torch.utils.data.WeightedRandomSampler(self.recency_sampler.prob_table.table, self.train_dataloader.dataset.data_size, replacement=True)
            return sampler
        ...
        for epoch in range(self.config.agent.epochs):
            ...
            ## recency bias
            if epoch < self.config.agent.warmup:
                set_sampler()
            else:
                sampler = set_sampler(epoch=epoch)
